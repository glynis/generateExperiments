...
outputdir <- "Starfish.exptfiles";
setwd("~/Dropbox/Sandcastle/RScripts");
fileName <- "starfish_finalized_stimuli.csv";
...
# set up columns
data <- subset(data, select=c(cond.name, blockindex, condcode, Condition, cond.cloze, good.bad, set, Listnum, Word1, Word2, Word3, Word4, Word5, Word6, Word7, Target, Continuation, Target.Prob)) ###

# trim 


# concatenate sentence frame
data$Frame <- (paste(trim(data$Word1), trim(data$Word2), trim(data$Word3), trim(data$Word4), trim(data$Word5), trim(data$Word6), trim(data$Word7), sep = " ", collapse = NULL))
...
addtarg <- function(row,blockfilename,question){
  ...
  triggers[length(triggers)] <- conditioncode*10;      # ensures sentence final trigger
...
}                                                      # requires that you delete all previous subj files before running
